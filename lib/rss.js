...
function rssCloudPing (urlServer, urlFeed, delaySeconds=undefined) { //6/5/15 by DW
	...
	function doPing () {
		...
		$.post (urlServer, {url: urlFeed}, function (data, status) {
			console.log ("rssCloudPing: urlFeed == " + urlFeed + ", status == " + status + ", " + secondsSince (whenstart) + " secs");
			});
		}
	if (delaySeconds === undefined) {
		...
		const ctMillisecs = Math.floor (delaySeconds * 1000);
		setTimeout (doPing, ctMillisecs);
		}
	}
function getRssEnclosureInfo (obj, callback) {
	const apiUrl = urlGetRssEnclosureInfo + encodeURIComponent (obj.enclosure.url);
	readHttpFileThruProxy (apiUrl, undefined, function (theText) {
		try {
			var jsontext = stringDelete (theText, 1, 9);
			jsontext = stringMid (jsontext, 1, jsontext.length - 1); 
			var jstruct = JSON.parse (jsontext);
			obj.enclosure.type = jstruct.type;
			obj.enclosure.length = jstruct.length;
			}
		catch (err) {
...
function buildRssFeed (headElements, historyArray) {
	...
	const localTimeFormat = "%A, %B %e, %Y at %l:%M %p"; //6/13/25 by DW
	function add (s) {
	...
	add ("<rss version=\"2.0\" xmlns:source=\"https://source.scripting.com/\"" + contentNamespaceDecl + facebookNamespaceDecl + ">"); indentlevel++ //7/2/23 by DW
	...
	//add header elements
		...
		add ("<source:localTime>" + formatDate (starttime, localTimeFormat) + "</source:localTime>"); //5/16/24 by DW
		//<source:self> -- 5/20/24 by DW
			if (headElements.urlSelf !== undefined) {
				add ("<source:self>" + encode (headElements.urlSelf) + "</source:self>"); 
				}
		//idWordpressSite -- 5/16/24 by DW
			if (headElements.idWordpressSite !== undefined) {
				add ("<site xmlns=\"com-wordpress:feed-additions:1\">" + encode (headElements.idWordpressSite) + "</site>");
				}
	...
	//add items
		...
		for (var i = 0; (i < historyArray.length) && (ctitems < maxitems); i++) {
			...
			if ((!getBoolean (headElements.flTitledItemsOnly)) || (item.title !== undefined)) { //2/22/16 by DW
				...
				//iidWordpressPost -- 5/16/24 by DW
					if (item.idWordpressPost !== undefined) {
						add ("<post-id xmlns=\"com-wordpress:feed-additions:1\">" + encode (item.idWordpressPost) + "</post-id>");
						}
